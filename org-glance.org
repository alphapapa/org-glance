         ...
         (implant (lambda (file-or-buffer scope-type)
                    ...
                    (with-current-buffer (get-buffer-create org-glance--scope-buffer-name)
                      ...
                      (let* ((entries (with-temp-buffer
                             ...
                             (org-parse-tree (org-element-parse-buffer 'headline))
                             (scope-trees (org-element-map org-parse-tree 'headline
                                            (lambda (headline)
                                              (and (= (org-element-property :level headline) 1)
                                                   (string= (org-element-property :title headline) scope-name)
                                                   (list
                                                    (org-element-property :begin headline)
                                                    (org-element-property :end headline)))))))

                        (if scope-trees
                            (loop for (begin end) in scope-trees
                                  do (save-excursion
                                       (goto-char begin)
                                       (org-map-entries (lambda () (message (org-element-property :title (org-element-at-point)))) nil 'tree)))
